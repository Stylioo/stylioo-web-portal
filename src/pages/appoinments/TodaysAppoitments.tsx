...
function TodaysAppoitments() {
    ...
    const getAllProducts = async () => {
        setIsLoading(true)
        const res = await axios.get('/appointment?range=today')
        if (res.data.success) {
    ...
    const searchProducts = async () => {
        setIsLoading(true)
        const res = await axios.get(`/appointment/search?term=${searchTerm}&range=todays`)
        if (res.data.success) {
            if (res.data.data.length === 0) {
                setappointments([])
                setSnakbarAlertMessage({ type: 'info', message: 'No data found' })
...
export default TodaysAppoitments