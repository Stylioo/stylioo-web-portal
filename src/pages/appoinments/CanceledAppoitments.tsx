...
function CanceledAppoitments() {
    ...
    const getAllProducts = async () => {
        setIsLoading(true)
        const res = await axios.get('/appointment?type=canceled')
        if (res.data.success) {
    ...
    const searchProducts = async () => {
        setIsLoading(true)
        const res = await axios.get(`/appointment/search?term=${searchTerm}&type=canceled`)
        if (res.data.success) {
            if (res.data.data.length === 0) {
                setappointments([])
                setSnakbarAlertMessage({ type: 'info', message: 'No data found' })
...
export default CanceledAppoitments